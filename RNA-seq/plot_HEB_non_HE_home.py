...
# 手动设定组织文件顺序
files = [
    "no_HE_homo_bla.count.tmp",
    "no_HE_homo_bra.count.tmp",
    "no_HE_homo_eye.count.tmp",
    "no_HE_homo_gill.count.tmp",
    "no_HE_homo_heart.count.tmp",
    "no_HE_homo_int.count.tmp",
    "no_HE_homo_key.count.tmp",
    "no_HE_homo_liver.count.tmp",
    "no_HE_homo_mus.count.tmp",
    "no_HE_homo_skin.count.tmp"
]
...
for i, file in enumerate(files):
    ...
    # 创建断轴子图
    ax = brokenaxes(ylims=((0, 1600), (1700, 4500)),
...
for i, ax in enumerate(broken_axes_list):
    ax.axs[0].set_yticks([4500])
    ax.axs[1].set_yticks([0, 500, 1000, 1500])
...
plt.savefig("HEB_10_tissues_brokenaxes.pdf", dpi=300)
plt.savefig("HEB_10_tissues_brokenaxes.png", dpi=300)
